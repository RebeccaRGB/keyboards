...
public class KeyCapStyle extends KeyCapMold {
	private static final float DECIMAL_PLACES = 1000;
	
	public static final KeyCapStyle KCAP_KBIT = new KeyCapStyle(
		5, 5, 5,
		new Color(0xCCCCCC),
		new float[][] {
			{ 0, 0, 0, 0, 0, 0 },
	...
	public static final KeyCapStyle KCAP_ICL8 = new KeyCapStyle(
		7, 7, 7,
		new Color(0xCCCCCC),
		new float[][] {
			{ 0, 0, 0, 0, 0, 0 },
	...
	private final int pathInset;
	private final Color defaultColor;
	private final float[][] layerColors;
	private final Shape[][] layerShapes;
	
	public KeyCapStyle(
		int pathWidth, int pathHeight, int pathInset,
		Color defaultColor, float[][] layerColors, String[][] layerPaths
	) {
		...
		this.layerShapes = ShapeUtilities.fromSVGPath(layerPaths);
	}
	...
	public String layeredAreaToSVGPaths(Area[] areas) {
		...
		for (int li = 0; li < layerShapes.length; li++) {
			...
			Color cc = ColorUtilities.multiplyAdd(defaultColor, c[0], c[1], c[2], c[3], c[4], c[5]);
			String ccc = "#" + Integer.toHexString(0xFF000000 | cc.getRGB()).substring(2);
			String path = ShapeUtilities.toSVGPath(shape, null, DECIMAL_PLACES);
			sb.append("<path fill=\"");
			sb.append(ccc);
			sb.append("\" d=\"");
			sb.append(path);
	...
	public Insets getInsets() {
		return new Insets(pathInset, pathInset, pathInset, pathInset);
	}
	
	public String getTextColor() {
		int rgb = ColorUtilities.contrastingColor(defaultColor).getRGB();
		return "#" + Integer.toHexString(0xFF000000 | rgb).substring(2);
	}
	...
	*/
	...
	public LayeredObject createLayeredObject(Shape shape, Color color, Float opacity) {
		...
		Area[] areas = layeredAreaFromShape(null, shape);
		Color[] colors = ColorUtilities.multiplyAdd(color, layerColors);
		LayeredObject[] layers = new LayeredObject[areas.length];
		for (int li = 0; li < areas.length; li++) {
			layers[li] = new LayeredShape(areas[li], colors[li], null);
		}
		return new LayeredGroup(Arrays.asList(layers), null, null);
	}
	
	public Shape createTopTextArea(Shape shape) {
		return ShapeUtilities.contract(shape, pathInset);
	}
	...
	public static void main(String[] args) {
		KeyCapStyle mold = KeyCapStyle.KCAP_ICL8;
